    ...
    if (data) {
       ...
       document.getElementById("statusResult").innerHTML = "Route Found";
        ...
        document.getElementById("result").innerHTML = `
            ...
            <p><b>Building:</b> ${data.building}</p>

            <p><b>Floor:</b> ${data.floor}</p>

            <p><b>Route:</b><br>${data.route}</p>
        `;
...
function drawRoute(routeArray){
    ...
    routeArray.forEach(junction => {
        points += `${junctionCoords[junction].x},${junctionCoords[junction].y} `;
    });
...
for(let id in junctionCoords){

    let point = document.createElement("div");

    point.className = "junction";

    point.style.left = junctionCoords[id].x + "px";
    point.style.top = junctionCoords[id].y + "px";

    point.innerHTML = id;

    document.querySelector(".map-area").appendChild(point);
}
